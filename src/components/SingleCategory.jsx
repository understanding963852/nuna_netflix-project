...
function SingleCategory({ genres }) {
  ...
  async function getDataByCategory(genres, pageNum) {
    ...
    let res = await axios.get(
      `https://api.themoviedb.org/3/movie/now_playing?api_key=${APIkey}&language=en-US&page=${pageNum}&with_genres=${genresResult}`
    );
  ...
  return (
    <li onClick={() => getDataByCategory(genres, 1)} className="genres-btn">
      {genres}
    </li>
  );
}
