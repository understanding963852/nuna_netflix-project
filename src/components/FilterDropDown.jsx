...
function FilterDropDown(props) {
  ...
  let { category, movie, changePage, page, totalPage, filterType } =
  ...
  async function getDataByFilter(filterType, pageNum) {
    ...
    let res = await axios.get(
      `https://api.themoviedb.org/3/discover/movie?api_key=${APIkey}&language=en-US&sort_by=${filterType}&include_adult=true&include_video=false&page=${pageNum}`
    );
